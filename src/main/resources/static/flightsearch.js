...
function onSubmit(e) {
  ...
  let leavingOn = form.elements[4].value.split('/');
  leavingOn = leavingOn[2] + '/' + leavingOn[1] + '/' + leavingOn[0];

  let returningOn = form.elements[5].value.split('/');
  returningOn = returningOn[2] + '/' + returningOn[1] + '/' + returningOn[0];
  ...
  //let url = `http://localhost:8080/search?returnTrip=${returnTrip.checked}&onewayTrip=${onewayTrip.checked}&departFrom=${departFrom}&arrivingAt=${arrivingAt}&leavingOn=${leavingOn}&returningOn=${returningOn}&adults=${adults}&children=${children}&classType=${classType}`;
  ...
  xhr.onreadystatechange = function() {
    ...
    if (xhr.readyState === DONE) {
      ...
      if (xhr.status === OK) {
        ...
        arrData = JSON.parse(resp.result).data;
        const data = JSON.parse(resp.result).data.map(flight => ({departure: flight.local_departure, arrival: flight.local_arrival, price: flight.price}));
        let table = '<table id="tblData">'
        table += '<thead><tr> <th>Departure</th> <th>Arrival</th> <th>Price</th> </tr></thead>'
        table += '<tbody>'
        data.forEach((flight,index) => {
          table += '<tr> <td>'+ new Date(flight.departure).toLocaleString() +'</td> <td>'+ new Date(flight.arrival).toLocaleString() +'</td> <td>'+ flight.price +'</td> </tr>';
        })
        table += '</tbody>'
        table += '</table>'
        _('confirm').innerHTML = txt + table;
        addClickEvent();
      }
...
function addClickEvent()
{
	...
	if(tbl)
	{
		var arrRows = tbl.querySelectorAll('tbody tr');
		if (arrRows) 
		{
		  for (var i = 0; i < arrRows.length; i++) 
		  {
			  var row = arrRows[i];
			  row.addEventListener("click",rowClick.bind(null,arrData[i],i));
		  }
		}
	}	
}

function rowClick(item,index,event)
{
	if(localStorage["airline-app-session"])
	{
		console.log(item);
		console.log(index);
		saveBooking(item,JSON.parse(localStorage["airline-app-session"]))
	}
	else
	{
...
function saveBooking(data,userInfo)
{
	var item = {bookingToken: data.booking_token,cityFrom: data.cityFrom,cityTo: data.cityTo,bookingId: data.id,
			local_arrival: data.local_arrival,local_departure: data.local_departure,price: data.price,userId: userInfo.id};
	...
	xhr.onload = () => {
	    if (xhr.status >= 200 && xhr.status < 300) {
	        ...
	        {
	        	alert("You booking has been saved sucessfully");
	        	window.location.href = "booking.html";
	        }
	    }
	};
	xhr.open('POST', "http://localhost:8080/savebooking");
